...
async def substantiate_claims(
    ...
    async def process_chunk_substantiation(chunk_idx, chunk):
        try:
            claims_for_chunk = (
                state["claims_by_chunk"][chunk_idx]
                if chunk_idx < len(state["claims_by_chunk"])
                else None
            )

            if (
                not claims_for_chunk
                or not hasattr(claims_for_chunk, "claims")
                or not claims_for_chunk.claims
            ):
            ...
            for claim in claims_for_chunk.claims:
                needs_substantiation = getattr(
                    claim, "needs_substantiation", False
                ) or getattr(claim, "needsSubstantiation", False)
            ...
            for claim_index, claim in enumerate(claims_to_substantiate):
                try:
                    ...
                    logger.error(
                        f"Error substantiating claim {claim_index} in chunk {chunk_idx}: {e}"
                    )
                    continue
    ...
    if target_indices is not None:
        ...
        logger.info(
            f"substantiate_claims: Selective processing of chunks {target_indices}"
        )
    else:
    ...
    tasks = [
        process_with_semaphore(chunk_idx, chunk)
        for chunk_idx, chunk in chunks_to_process
    ]
    ...
    for (chunk_idx, _), result in zip(chunks_to_process, chunk_results):
        if isinstance(result, Exception):
            ...
            logger.warning(
                f"Unexpected result type for chunk {chunk_idx}: {type(result)}"
            )
            substantiation_chunk = ClaimSubstantiationChunk(substantiations=[])
    ...
    logger.info(
        f"substantiate_claims: Completed substantiation for {len(final_substantiations)} chunks"
    )
    return {
        "chunks": [
            state["chunks"][index].model_copy(
                update={"substantiations": final_substantiations[index]}
            )
            for index in range(len(final_substantiations))
        ]
    }
