...
async def generate_addendum(state: ClaimSubstantiatorState) -> ClaimSubstantiatorState:
    ...
    if not state.config.run_live_reports:
        logger.info(
            f"generate_addendum ({state.config.session_id}): skipping (run_live_reports is False)"
        )
        return {}
    ...
    if agents_to_run and "live_reports" not in agents_to_run:
        logger.info(
            f"generate_addendum ({state.config.session_id}): Skipping (live_reports not in agents_to_run)"
        )
        return {}
    ...
    if not records:
        logger.info(
            f"generate_addendum ({state.config.session_id}): no live report records, skipping"
        )
        return {}
    ...
    addendum = await addendum_generator_agent.ainvoke(prompt_kwargs)
...
if __name__ == "__main__":
    ...
    async def _main():
        ...
        from lib.agents import addendum_generator as ag
        ...
        ag.addendum_generator_agent.ainvoke = _fake_ainvoke

        delta = await generate_addendum(state)
        addendum = delta.get("addendum")
        print(addendum.model_dump_json(indent=2))
