...
def _reconstruct_bibliography_item(ref_data):
    ...
    return BibliographyItem(
        ...
        has_associated_supporting_document=ref_dict.get(
            "has_associated_supporting_document", False
        ),
        index_of_associated_supporting_document=ref_dict.get(
            "index_of_associated_supporting_document", -1
        ),
        name_of_associated_supporting_document=ref_dict.get(
            "name_of_associated_supporting_document", ""
        ),
    )


def _reconstruct_state_from_result_dict(
    ...
    state: ClaimSubstantiatorState = {
        ...
        "supporting_files": _reconstruct_supporting_files(
            original_result.get("supportingFiles", [])
        ),
        "chunks": original_result.get("chunks", []),
        "references": [
            _reconstruct_bibliography_item(ref)
...
async def reevaluate_single_chunk(
    ...
    return updated_state["chunks"][chunk_index]
