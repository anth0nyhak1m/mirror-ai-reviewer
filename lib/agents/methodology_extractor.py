...
class MethodologyExtractionResponse(BaseModel):
    methodology: str = Field(
        description=(
            "A concise but detailed description of the methodology used in the document "
            "to obtain its results. This should focus on procedures, data, models, "
            "experimental setups, and analysis workflows, and avoid background or interpretation."
        )
    )
...
2. Identify and describe the **procedures and processes** used to obtain the results, including as relevant:
   - Data sources, datasets, and sampling strategy
   - Experimental or observational setup (equipment, conditions, interventions, groups)
   - Computational or analytical methods (algorithms, models, estimators, simulations)
   - Training, fitting, or calibration procedures
   - Evaluation or validation procedures (metrics, baselines, statistical tests)
...
- A **coherent, stand-alone narrative** (not just a bullet list) of approximately **300–700 words**.
- Written in clear prose that could be read by a technically literate researcher unfamiliar with the paper.
- Focused on **what was actually done** to generate the results, not on why it matters or how it compares to prior work.

When important details are truly missing from the provided text (e.g., sample size, exact hyperparameters, full experimental conditions), explicitly indicate this with phrases like:
...
class MethodologyExtractorAgent(LangChainAgent):
    ...
    description = (
        "Read a research document and extract a concise but detailed description of "
